...
class Proxy():
    ...
    def __init__(self, zkserver="10.0.0.1", in_bound=5555, out_bound=5556):
        ...
        self.my_path = self.zk.create(
            self.elect_candidate_path, value=b'', sequence=True, ephemeral=True, makepath=True)
        ...
        if self.zk.exists(self.leader_path) is None:
            self.zk.create(self.leader_path, value=b'',
    ...
    def start(self):
        ...
        if self.my_path.endswith(self.candidate_list[0]):
            # leader node takes /election/client_sessionid_xxxxxxxx
            ...
            # set leader value to current node ip
            self.zk.set(self.leader_path, encoded_ip)
            # set candidate value to current node ip
            ...
            # IM HERE - if an ip in re
                ...
                if not self.zk.get_children(self.replica_root_path):
                    ...
                    broker_num = min(
                        ceil(len(self.topics)/self.TOPIC_THRESHOLD), standby_num)
                    for i in range(broker_num):
                        broker_ip = self.zk.get(
                            self.replica_standby_path + "/" + children[i])[0]
                        self.zk.create(
                            self.replica_root_path + "/" + children[i], value=broker_ip, sequence=True, ephemeral=True)
            ...
            def topic_watch(children):
                ...
                for child in children:
                    ...
                    if val['topic'] not in self.pub_data.keys():
                        self.pub_data.update({val['topic']: {val['pub_ip']: {
                                             'strength': val['strength'], 'history': val['history'], 'broker_ip': "", 'publications': []}}})

                    elif val['pubid'] not in self.pub_data[val['topic']].keys():
                        self.pub_data[val['topic']].update({val['pub_ip']: {
                                                           'strength': val['strength'], 'history': val['history'], 'broker_ip': "", 'publications': []}})
                ...
                if not self.replicas:
                    ...
                    try:
                        ...
                        if self.topics:
                            new_repl_cnt = max(
                                1, ceil(len(self.topics)/self.TOPIC_THRESHOLD))
                            old_repl_cnt = len(
                                self.zk.get_children(self.replica_root_path))
                            print(f'  - Total Workers Needed: {new_repl_cnt}')
                            print(
                                f'  - Active Workers Existing: {old_repl_cnt}')
        ...
        # Znodes exist earlier in candidate list - Current node becomes FOLLOWER
            ...
            def proxy_watcher(data, stat):
                print(
                    f"#### Backup Load Balancer: Leader's Publisher Reqistry Updated ####")
                if data is not None:
                    try:
                ...
                def proxy_watcher(data, stat, event):
                    ...
                    if event is not None:
                        if event.type == "DELETED":
                            ...
                            # blocks until wins or canceled
                            election.run(self.won_election)
            ...
            prev_node = self.zk.exists(
                self.elect_root_path + "/" + prev_node_path, watch_prev_node)
                    ...
                    try:
                        ...
                        if self.topics:
                            ...
                            old_repl_cnt = len(
                                self.zk.get_children(self.replica_root_path))
                            print(
                                f"  - New - Total Workers Needed: {new_repl_cnt}")
                            print(new_repl_cnt)
                            print(
                                f"  - Existing - Total Workers Existing: {old_repl_cnt}")
                            print(old_repl_cnt)
                            if new_repl_cnt > len(self.replicas):
                                print("  **Need Additional Workers**")
        ...
        try:
            if add_this == 'publisher':
                if topic not in self.pub_data.keys():
                    self.pub_data.update({topic: {pubid: {
                                         'strength': strength, 'history': history, 'broker_ip': "", 'publications': []}}})
                elif pubid not in self.pub_data[topic].keys():
                    self.pub_data[topic].update(
                        {pubid: {'strength': strength, 'history': history, 'broker_ip': "", 'publications': []}})
            elif add_this == 'publication':
                stored_publication = publication + '--' + str(time.time())
                self.pub_data[topic][pubid]['publications'].append(
                    stored_publication)
    ...
    def replicate_data(self):
        ...
        while self.isleader is False:
            ...
            try:
                ...
                print(
                    f'- received replica data from leader: {recv_pushed_pub_data}')
            ...
            if msg_type == 'register':
                self.update_data('publisher', pubid, topic,
                                 strength, history, '')
    ...
    def update_replicas(self, rep_count):
        ...
        print(f"Replica Root Path: {self.replica_root_path}")
        self.replicas = self.zk.get_children(self.replica_root_path)
        time.sleep(1)
        ...
        print(f"Replica Standby Path: {self.replica_standby_path}")
        ...
        print(f"Changes: {change_rep_num}")

        # create
        if change_rep_num > 0:
            print("change_rep_num")
            if len(self.standbys) - len(self.replicas) >= change_rep_num:
                for i in range(1, change_rep_num + 1):
                    ...
                    tmp_ip = self.zk.get(
                        path=self.replica_standby_path + "/" + self.standbys[crnt_stdby_idx + i])[0]
                    # print(f'{i} -new lb broker ip: {tmp_ip}')
                    path = self.replica_root_path + "/" + \
                        self.standbys[crnt_stdby_idx + i]
                    print(path)
                    self.zk.create(path, ephemeral=True, value=tmp_ip)
            else:
    ...
    def distribute_topics_to_replicas(self):
        ...
        for i in range(0, replica_count):
            rep_ip = self.zk.get(self.replica_root_path +
                                 "/" + self.replicas[i])[0]
            ...
            for j in range(idx * self.TOPIC_THRESHOLD, min(self.TOPIC_THRESHOLD + i * self.TOPIC_THRESHOLD, topic_count)):
                ...
                if self.topics[j] in self.pub_data:
                    ...
                    for t in self.pub_data[self.topics[j]]:
                        self.pub_data[self.topics[j]
                                      ][t]["broker_ip"] = rep_ip.decode('utf-8')
                        print(
                            f"  - Assigned Worker for {self.topics[j]}: {rep_ip.decode('utf-8')}")
                else:
...
class Publisher():
    ...
    def start(self):
        ...
        if not self.zk.exists(self.path):
            pub_val = {"pubid": self.pubid, "pub_ip": self.ip,
                       "strength": self.strength, "history": self.history, "topic": self.topic}
            b_pub_val = str(pub_val).encode('utf-8')
            self.zk.create(self.path, value=b_pub_val,
                ...
                if data is not None:
                    try:
                        ...
                        self.socket.send_string(
                            f'register {self.pubid} {self.topic} {self.strength} {self.history}')
                    except:
                        print(
                            "...Waiting on Load Balancer to update publisher registry")
        else:  # FLOOD MODE
            ...
            def proxy_watcher(data, stat):
                ...
                if data is not None:
                    try:
                        ...
                        print(
                            f"  - List of Worker's Strength for Topic: {mytopic}")
                        print(tmp_strength)
                        print(
                            f"\n  - List of Worker's History for Topic: {mytopic}")
                        print(tmp_history)
                        meets_hist_req = dict(
                            (k, v) for k, v in tmp_history.items() if v >= int(self.history))
                        ...
                        print(
                            "Publisher with Highest Strength Meeting History Requirements")
